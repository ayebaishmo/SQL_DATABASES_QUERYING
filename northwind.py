...
'''expensive_items: What are the ten most expensive items
(per unit price) in the database? Please return all
columns in the table, not just'''
expensive_items = '''
SELECT *
FROM Product
...
'''avg_hire_age: What is the average age of an employee at the
time of their hiring? (Hint: a lot of arithmetic works with dates.)'''
avg_hire_age = '''
SELECT
...
'''ten_most_expensive: What are the ten most expensive
items (per unit price) in the database and their suppliers? '''
ten_most_expensive = '''
...
'''largest_category: What is the largest
category (by number of unique products in it)?'''
largest_category = '''
